...
public enum SessionFormat: String, Sendable, Codable {
    case talk
    case workshop
    case panel
    case keynote
...
public struct Session: Sendable, Codable, Identifiable, FetchableRecord, PersistableRecord {
    ...
    /// Format of the session (stored as string for Codable compatibility)
    private var _format: String
    
    /// Session format as an enum
    public var format: SessionFormat {
        get { SessionFormat(rawValue: _format) ?? .talk }
        set { _format = newValue.rawValue }
    }
    
    /// Difficulty level (stored as string for Codable compatibility)
    private var _difficultyLevel: String
    
    /// Difficulty level as an enum
    public var difficultyLevel: DifficultyLevel {
        get { DifficultyLevel(rawValue: _difficultyLevel) ?? .all }
        set { _difficultyLevel = newValue.rawValue }
    }
    ...
    public init(
        ...
        self._format = format.rawValue
        self._difficultyLevel = difficultyLevel.rawValue
        self.track = track
        self.tags = tags
